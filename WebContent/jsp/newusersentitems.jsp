...
<title>UNIVERSITY HELPDESK | SENT ITEMS</title>
</head>
<body>
...
<table width="100%">
<tr>
<th style="top: 0%;left: 43%;position: absolute;font-size: 40px;color: blue;"> OUTBOX </th>
...
<div style="top: 5%;left: 0%;width: 100%;height: 90%;position: absolute;font-size: 18px">


<form action="/UniversityHelpdesk/NewUserSentitem" method="post">
...
String quer="select * from message where senderid=? and sstatus=?";
...
while(rs1.next())
{	%>
	<div style="top: 0%;left: 2%;width: 20%;height: 20%;position: absolute">
	<%
...
catch(SQLException se)
{
	...
	String e=rs.getString("recid");
	String f=rs.getString("subject");
	%>
	
	<tr><td><input type="checkbox" name="chk" value=<%=d %>></td><td><%=e %></td><td><a href="/UniversityHelpdesk/jsp/newusershowmsg.jsp?msid=<%=rs.getString("msgid") %>&recid=sent"><%=f %></a></td><td><%=rs.getString("date") %></tr>
	<%}
}
catch(SQLException se)
{
